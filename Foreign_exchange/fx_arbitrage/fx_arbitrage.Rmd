...
-  *Uncovered interest arbitrage* is a inaccurate name, because the activity is *not* an arbitrage. The trade is uncovered, and so there is exposure —- sometimes significant —- to FX risk.
...
<!---
##  OLD Triangular Arbitrage Calculator
...
sidebarLayout(
    sidebarPanel(
        ...
        numericInput("usdeur", "USD/EUR", value = 1.20),
        numericInput("usdgbp", "USD/GBP", value = 1.50),
        numericInput("eurgbp", "EUR/GBP", value = 1.30),
        radioButtons("currency.p", "Currency to State Profit", choices = c("EUR", "USD"), selected = "EUR")
        ),
    mainPanel(
        renderPlot({
            cross <- input$usdgbp / input$usdeur
            eur.profit <- abs(cross - input$eurgbp)
            usd.profit <- eur.profit * input$usdeur
        
            plot(0, ylim = c(0,1), xlim = c(0,1), type = "n", xaxt = "n", yaxt = "n", ylab = "", xlab = "")
            if(input$currency.p == "EUR"){
                text(x = 0.5, y = 0.5, labels = paste("EUR", round(eur.profit, 2), sep = " "), cex = 5)
            } else {
                text(x = 0.5, y = 0.5, labels = paste("USD", round(usd.profit, 2), sep = " "), cex = 5)
            }
        })
...
headerPanel("Triangular Arbitrage"),
sidebarPanel(
    selectInput("test2", "Test", choices = c("Choice 1" = "1", "Choice 2" = "2"), selected = "1")
    ),
mainPanel(
    grVizOutput('diagram' )
    )
...
Click [here](https://github.com/FinancialMarkets/5MinuteFinance/blob/master/Foreign_exchange/fx_arbitrage/fx_arbitrage.Rmd) to see the code and collaborators for this presentation.
